...
1. The next instructions assume that you are at the same level of the `pbmm-on-gcp-onboarding` folder. Change into `3-networks-dual-svpc` folder, copy the Terraform wrapper script and ensure it can be executed.

   ```bash
   cd pbmm-on-gcp-onboarding/3-networks-dual-svpc
